...
namespace Particles {
	void Render() {
		...
		if (!initialized)
		{
			for (int i = 0; i < numParticles; ++i)
			{
				...
				particleVelocities[i] = ImVec2(
					(rand() % 1) == 0 ? static_cast<float>((rand() % 25)) : static_cast<float>((rand() % -25)),
					(rand() % 1) == 1 ? static_cast<float>((rand() % 25)) : static_cast<float>((rand() % -25))
				);
		...
		for (int i = 0; i < numParticles; ++i)
		{
			...
			ImU32 particleColour = ImGui::ColorConvertFloat4ToU32(ImColor(1.f, 1.f, 1.f, 1.f));
