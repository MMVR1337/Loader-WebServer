...
void startServer(crow::SimpleApp& app) {
    consoleLogs.push_back("Сервер запускается...");
    app.port(443)
...
void stopServer(crow::SimpleApp& app) {
    consoleLogs.push_back("Сервер останавливается...");
    app.stop();
    serverRunning = false;
    consoleLogs.push_back("Сервер остановлен.");
}
...
void Init_Hwnd() {
    ...
    hwnd = ::CreateWindowExW(
        ...
        L"winFLSUHIloapo97af4",          // Заголовок окна
        WS_POPUP,                        // Стили окна
        window.x,                        // Позиция X
        window.y,                        // Позиция Y
        screen.x,                        // Ширина
...
void server_thread() {
    ...
    try {
        fs::create_directories("./files/data"); // Создаём папку, если отсутствует
        ...
        if (!fs::exists(key_path)) {
            ...
            file << "{}"; // Инициализируем пустым JSON-объектом
            file.close();
        }
        ...
        db.init(); // Предполагается, что db.init() загружает keys.json и users.json
...
// API маршруты
        ...
        // Маршруты для HTML-страниц без расширения .html
        CROW_ROUTE(app, "/")([]() {
            crow::response res;
        ...
        CROW_ROUTE(app, "/sites/default/private/files/backup_migrate/scheduled/test.txt")([]() {
            crow::response res;
            res.code = 302;
            res.set_header("Location", "https://www.youtube.com/watch?v=dQw4w9WgXcQ"); // URL для перенаправления
            return res;
            });
        ...
        CROW_ROUTE(app, "/styles.css")
            ...
            if (fs::exists(filepath)) {
                crow::response res;
                res.set_static_file_info(filepath);
                res.add_header("Content-Type", "text/css"); // Указываем правильный MIME-тип
                return res;
            }
            else {
                crow::response res(404);
